...
export default {
      ...
      KaryawanService.addKaryawan(payload)
        .then(({ data: { result, message } }) => {
          if (message == "OK") {
            ...
            if (this.id) {
              this.$router.replace({ name: KARYAWAN.DETAIL, params: { karyawanId: this.id } });
            } else {
              this.$router.replace({ name: KARYAWAN.DETAIL, params: { karyawanId: result.karyawan_id } });
            }
  ...
  mounted() {
    this.getListProjek();
    this.getListDivisi(() => this.getDetail());
  },
  watch: {
    payload: {
      handler(val) {
